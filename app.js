...
testUpdate.innerText = 'This is the first to see if it works'
console.log('WORKLET_PATH_PROD', WORKLET_PATH_PROD)
...
const startIpc = async () => {
    try {
        ...
        const req = myrpc.request(API.TESTCOMMAND)
        req.send(JSON.stringify({ 'data': "adfadada" }))
        const res = await req.reply('utf8')
        const parsedRes = JSON.parse(res)

        testUpdate.innerText = 'Response: ' + parsedRes.message
        console.log('response from oniri-core:', parsedRes)
    } catch (error) {
        const testUpdate = document.getElementById('testUpdate')
        console.error('Error setting up IPC:', error)
        testUpdate.innerText = 'Response: ' + JSON.stringify(error)
    }
}
